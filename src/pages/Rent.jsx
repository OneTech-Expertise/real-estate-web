...
export default function Rent() {
  ...
  const filteredProperties = useMemo(() => {
    let result = properties.filter((p) => p.purpose === 'Rent')
    ...
    if (filters.furnished === 'true') result = result.filter((p) => p.furnished)
  ...
  const resetFilters = () => {
    setFilters({ purpose: 'Rent', type: '', location: '', minPrice: '', maxPrice: '', bedrooms: '', bathrooms: '', area: '', furnished: '', featured: '' })
  }
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Find quality rental properties across Lahore - from apartments to houses."
        breadcrumb={[{ label: 'Home', to: '/' }, { label: 'Rent' }]}
      />
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <p className="text-sm text-muted">
                    <span className="font-semibold text-primary">{filteredProperties.length}</span> properties for rent
                  </p>
                </div>
                <select
                  value={sortBy}
